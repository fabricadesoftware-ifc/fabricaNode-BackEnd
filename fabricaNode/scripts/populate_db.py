...
def random_area(n):
    return [(fake.word(),) for _ in range(n)]  # A vírgula transforma a palavra em uma tupla
...
def random_estado(n, pais_ids):
    return [(fake.state_abbr(), fake.state(), random.choice(pais_ids)) for _ in range(n)]
...
def random_autor(n, cidade_ids):
    return [(fake.first_name(), fake.last_name(), fake.email(), fake.date_of_birth().isoformat().replace('-', ''), random.choice(cidade_ids)) for _ in range(n)]
...
def random_publicacao(n, editora_ids):
    return [(fake.sentence(), fake.year(), random.choice(editora_ids), fake.sentence(), fake.text(), fake.url(), random.choice(['J', 'C', 'L']), random.choice(['INT', 'NAC', 'REG', 'LOC']), random.choice(['RESUM', 'FULLP', 'LIVRO', 'CAPLI', 'TESES', 'DISSE', 'RELAT', 'MONOG', 'OUTRO'])) for _ in range(n)]

# Função para gerar dados de associação de autores com publicações
def random_publicacao_autores(publicacao_ids, autor_ids):
    return [(random.choice(publicacao_ids), random.choice(autor_ids)) for _ in range(len(publicacao_ids))]
...
def random_publicacao_keywords(publicacao_ids, keyword_ids):
    return [(random.choice(publicacao_ids), random.choice(keyword_ids)) for _ in range(len(publicacao_ids))]
